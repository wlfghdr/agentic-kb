...
- **Multi-harness `/kb` install parity (Codex CLI, Gemini CLI, Kiro IDE)** — `scripts/install.py` now supports `--target codex`, `--target gemini`, and `--target kiro`, each writing the native file that registers `/kb` as a first-class slash command on that harness. Codex: markdown in `~/.codex/prompts/kb.md` (global-only per Codex design). Gemini: generated TOML wrapper at `.gemini/commands/kb.toml` (or global) whose `prompt` field embeds the canonical command body. Kiro: markdown at `.kiro/agents/kb.md` — Kiro's custom agents double as slash commands. `detect_targets()` probes all six harnesses; `--target all` fans out to every installer-supported harness. `scripts/test_install.py` grows four regression cases. README and `docs/REFERENCE.md` document the per-harness file paths; `kb-setup` Q10 offers the six harnesses as IDE targets. Cursor/Windsurf (rules-only) and Aider/Pi/raw Claude (no slash-command hook) stay flagged as not-feasible.
