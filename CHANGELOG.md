...
- **`kb-setup` Step 7: first-win + next-steps shortlist (#99, #101)** — setup no longer ends with a read-only "Setup: OK" message. After verification, the wizard runs an optional bootstrap capture (default yes, paste URL/file/text → full evaluation gate → finding written → `index.html` regenerated → the artifact path is named back to the user), then emits a curated 3–5 command shortlist tailored to the chosen adoption stage and audience (Stage 1 solo, Stage 1 multi-user, Stage 2 solo, Stage 2 multi-user, Stage 3) plus a short closing summary that names at most 3 next commands. Adopters leave setup with a visible artifact they can open in a browser, not with a flat 33-command list.
- **README "Most used (start here)" section (#101)** — a five-command shortlist (`/kb capture`, `/kb note meeting`, `/kb decide`, `/kb start-day`, `/kb status`) appears above the full Stable core flows list. New readers do not need to learn the full 33-command surface to get value; they can come back to it as their workflow asks for it.
