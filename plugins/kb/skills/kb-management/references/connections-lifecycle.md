...
|------|---------------|-----------------|
| `product-repos` | Git repos where the user reads but may not write | Product code repos, shared config repos, open-source dependencies |
| `trackers` | Issue or project trackers | GitHub Issues, Jira, Linear, exported CSV |
...
| `reference-mode` | `link` = cite the source; `inline` = embed a summary; `none` = record watermark only |
...
Connections are declared during the setup interview (block 9). The setup skill:

1. asks which product repos and trackers the layer should follow,
...
5. runs an initial `/kb digest connections` (dry-run unless the user confirms) to set the first watermark.

No connections means the `connections:` block is written with empty lists and `reference-mode: link`. The block stays in the config so a later setup rerun can add connections without a merge conflict.
...
3. records the HEAD commit SHA or export timestamp as the watermark in `_kb-references/strategy-digests/.last-digest` (or per-source file `<name>.watermark`),
4. logs `digest-connections | personal | <name> | initial digest, watermark set`.
...
2. fetches only changes since that watermark (commits, issues opened/closed, project column moves),
3. runs each delta through the evaluation gate against the layer's active workstream themes,
4. writes a delta digest finding when the gate score is 1 or higher — `_kb-references/strategy-digests/YYYY/YYYY-MM-DD-<name>-delta.md`,
...
When `/kb` runs without arguments (triage scan), it compares each declared connection's current HEAD or latest export timestamp against the stored watermark. If any connection is ahead of its watermark, triage surfaces:

```
Connection drift: backend-api, jira-proj          → /kb digest connections
```
...
Write-back actions require explicit user confirmation before any update is posted. The skill never posts silently, even at automation level 3. Every write-back is logged with `digest-connections-writeback` + the target identifier and action taken.
...
2. optionally delete the associated watermark file(s),
...
- [`../../../../../plugins/kb/skills/kb-setup/SKILL.md`](../../kb-setup/SKILL.md) — interview block 9

## Changelog

| Date | What changed | Source |
|------|-------------|--------|
| 2026-04-25 | Initial reference covering connection kinds, config shape, setup flow, digest lifecycle, watermark format, triage drift check, write-back, and disconnect | Deep spec-audit follow-up |
