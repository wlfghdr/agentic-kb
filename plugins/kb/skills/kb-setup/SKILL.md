...
7. if any tracker-backed primitive is configured, validate that each `primitive-storage.*.tracker` points to a declared tracker, that live adapters declare canonical CRUD capabilities, and that every mutation-capable token-only adapter names its required canonical `auth-env` source or the adapter documents an ambient authentication mechanism. Setup does not require an ambient session to be active: when it is unavailable, preserve the configured connection and let each mutation use the documented manual fallback until the user logs in. Also validate that required issue forms or type-mapping instructions exist for GitHub/Jira selections and that reserved connection-digest write-back remains disabled. A token-only adapter with no authentication source fails setup verification rather than completing with an unusable connection.
...
|------|-------------|--------|
| 2026-09-17 | Limited setup authentication failure to token-only adapters without a configured source; inactive ambient sessions now defer to the per-mutation manual fallback | PR #153 review |
