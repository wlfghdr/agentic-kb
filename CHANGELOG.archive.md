...
Pre-v5.0 release history for **agentic-kb**, split out of [`CHANGELOG.md`](CHANGELOG.md) to keep the active changelog focused on the current major line. This file is read-only memory; new entries always land in `CHANGELOG.md` under `[Unreleased]` first.

Versioning conventions (PATCH/MINOR/MAJOR) and the broader convention notes live in the active [`CHANGELOG.md`](CHANGELOG.md) header — not duplicated here.
