...
1. Read `_tasks/focus.md`.
2. Scan `_decisions/active/` — any due soon or blocked?
...
6. Scan `_inputs/` — anything unprocessed?
...
2. Move completed focus items → `_tasks/archive/YYYY-MM.md`.
...
3. Audit `_decisions/active/` — overdue? new evidence?
4. Audit `_tasks/backlog.md` for stale items (>14 days untouched).
...
1. Generate `_references/findings/YYYY-MM-DD-weekly-summary.md`.
...
- If user is unavailable at 15:00, queue output as an `_inputs/` item for next session.
